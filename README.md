...
| スクリプト | 役割 |
| --- | --- |
| `scripts\setup_daily_aqua_memo.bat` | リポジトリの clone / fetch / ブランチ checkout を補助する。`C:\Users\shuns\.claude\projects\daily_aqua_memo` に clone(既存なら更新)し、`feature/mvp-local-outbox` に切り替える |
...
- 既存 clone(`.git` あり)なら `git fetch` → `git switch feature/mvp-local-outbox` → `git pull --ff-only`
